...
class pos_order(models.Model):
    ...
    def load_ir_config_parameter(self):
        sys_param = self.env['ir.config_parameter'].sudo().search([('key', '=', 'google_api_key')], order='id desc',
                                                                  limit=1)
        if sys_param:
            ...
            if loyalty_setting_id:
                if loyalty_setting_id.points_based_on and order.get('loyalty_earned_point'):
                    point_vals = {
                        ...
                        'amount_total': (float(order.get(
                            'loyalty_earned_point')) * loyalty_setting_id.to_amount) / loyalty_setting_id.points
                    }
                ...
                if order.get('loyalty_redeemed_point'):
                    redeemed_vals = {
                        ...
                        'partner_id': res.partner_id.id,
                        'redeemed_amount_total': self._calculate_amount_total_by_points(loyalty_setting_id, order.get(
    ...
    def _calculate_amount_total_by_points(self, loyalty_config, points):
        return (float(points) * loyalty_config.to_amount) / (loyalty_config.points or 1)
