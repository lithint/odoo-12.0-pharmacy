    	...
    	renderElement: function(){
            ...
            this.$('#partial_pay').click(function(){
            	if(self.pos.get_order().get_client()){
            		var value = $('.create_picking_debit').find('i').hasClass('fa-toggle-on')
                    if(value){
                        self.pos.get_order().set_order_make_picking(value)
                    }
    	...
    	start: function(){
			...
			$('#delivery_mode').click(function(){
		    	...
		    	if($('#delivery_mode').hasClass('deliver_on')){
		    		if(lines.length > 0){
		    			var r = confirm("Are you sure you want to remove delivery order?");
		    			if(r){
		    				order.clear_cart();
		    				$('#delivery_mode').removeClass('deliver_on');
		    			}
		    		...
		    		if(lines.length > 0){
		    			var r = confirm("Are you sure you want to make delivery order?");
		    			if(r){
		    				if(!order.get_is_delivery()){
		    					var deliver_product_id = self.pos.config.delivery_product_id[0];
					    		var deliver_amt = self.pos.config.delivery_amount;
