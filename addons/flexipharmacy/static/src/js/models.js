odoo.define('flexipharmacy.models', function (require) {
	...
	models.load_fields("res.users", ['image_small','login','company_ids','access_ereceipt','access_quick_cash_payment',
	                                 ...
	                                 'access_pos_graph','access_x_report','access_pos_loyalty','is_delivery_user',
...
//        	return [['from_date','<=',current_date],['to_date','>=',current_date],['active','=',true]];
	...
	models.PosModel = models.PosModel.extend({
		initialize: function(session, attributes) {
		    /*Start Product-Customer Sync Code*/
            this.p_init();
            ...
            /*End Product-Customer Sync Code*/
            var self = this;
			this.product_list = [];
        ...
        synch_without_reload: function (change_detector) {
            ...
            $.when(indexedDB.get('products'), indexedDB.get('customers')).then(function (products, customers) {
                // order_by product
                products = indexedDB.order_by(products, self.get_model('product.product').order, 'esc');
                ...
                products_screen.product_list_widget = new ProductListWidget(products_screen, {
                    ...
                    product_list: self.db.get_product_by_category(0)
                });
            ...
            rpc.query({
                ...
                model.loaded = function (self, new_customers) {
                    ...
                    indexedDB.get('customers').then(function (customers) {
                        ...
                        // order_by
                        customers = indexedDB.order_by(customers, model.order||['display_name'], 'esc');
                        self.c_super_loaded(self, customers);
...
//            		  var change = this.get_total_paid() - this.getNetTotalTaxIncluded() - this.get_order_total_discount();
        ...
        set_deliver_mode: function(mode){
        	return this.set('delivery_mode',mode)
        },
