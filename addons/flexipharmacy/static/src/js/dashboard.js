odoo.define('flexipharmacy.dashboard', function (require) {
    ...
    var ProductExpiryDashboard = AbstractAction.extend(ControlPanelMixin, {
        ...
        location_expire: function(event){
            var self = this;
            self.do_action({
                   ...
                   target: 'new',
                   domain:[['state_check','=','near_expired'],['location_id','=',parseInt(event)]],
            });
        },

        categ_div_id : function (event){
            var self = this;
            self.do_action({
        ...
        open_near_expire: function (event) {
            var self = this;
            self.do_action({
                   name : _t('Nearly Expire Product'),
                   views: [[false, 'list'],[false, 'form']],
        ...
        renderElement: function () {
           ...
           .then(function(records){
               ...
               var userList = new List('warehouse', options, values);
                   $('#expire_in_60_days').text(records['60']);
                   $('#expire_in_30_days').text(records['30']);
                   $('#expire_in_15_days').text(records['15']);
                   $('#expire_in_10_days').text(records['10']);
                   $('#expire_in_5_days').text(records['5']);
                   $('#expire_in_tomorrow').text(records['1']);
                   ...
                   for(var data=0;data<records['category_near_expire'].length;data++){
                        var html = "<div class='col-md-4 col-sm-6 col-xs-12 ng-scope custome_hide' style='float: left;width: 750px;'>";
                        html += "<div class='content' data-categ_id='"+records['category_near_expire'][data]['id']+"'><div class='info-box'>";
                        ...
                        html += "<div class='wrimagecard-topimage_title'>";
                   ...
                   $(document).ready(function () {
                        ...
                        var vale = $(this).text()
                            ...
                            $('.content').parents('.custome_hide').addClass('hidden')
                        ...
                        $(function() {
                            ...
                            function cb(start, end) {
                                $('#reportrange span').html(start.locale('en').format('MMMM D, YYYY') + ' - ' + end.locale('en').format('MMMM D, YYYY'));
                                rpc.query({
                                    model: 'product.product',
                                    method: 'graph_date',
                                    args: [false,start.locale('en').format('YYYY-MM-D'),end.locale('en').format('YYYY-MM-D')],
                                    }, {async: false})
                                    .then(function (res) {
                                        var total_quantity = []
                                        var product_name = []
...
//                                        document.getElementById("aaa").innerHTML = "Nearly Expire Product Quantity Graph";
                                        ...
                                        $('#display_pie').removeClass('btn btn-default fa fa-pie-chart-o o_graph_button active');
                                        $('#display_pie').addClass('btn btn-default fa fa-pie-chart-o o_graph_button inactive');
                                        $('#display_bar').removeClass('btn btn-default fa fa-bar-chart-o o_graph_button inactive');
                                        $('#display_bar').addClass('btn btn-default fa fa-bar-chart-o o_graph_button active');
...
//                                        document.getElementById("aaa").innerHTML = "Nearly Expire Product Quantity Graph";
                                        $("#near_expiry_graph_panel").show();
                                        var $this = $(this);
                                        $('#display_bar').removeClass('btn btn-default fa fa-bar-chart-o o_graph_button active');
                                        $('#display_bar').addClass('btn btn-default fa fa-bar-chart-o o_graph_button inactive');
                                        $('#display_pie').removeClass('btn btn-default fa fa-pie-chart-o o_graph_button inactive');
                                        $('#display_pie').addClass('btn btn-default fa fa-pie-chart-o o_graph_button active');
                                        if (res.length >= 1){
                                        $('#product_graph_display').remove();
                                         $('#BarChart_canvas').append("<canvas id='product_graph_display'></canvas>");
