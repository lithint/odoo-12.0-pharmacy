odoo.define('aspl_pos_backend_dashboard.dashboard', function (require) {
    ...
    var POSDashboard = AbstractAction.extend(ControlPanelMixin, {
        ...
        top_items_by_sales : function(company_id){
            ...
            var start = moment().startOf(option).format('YYYY-MM-DD');
            var end = moment().endOf(option).format('YYYY-MM-DD');
        ...
        chart_journal : function(graph, data){
             AmCharts.makeChart("journal_line_chart", {
              ...
              "theme": "light",
              "titles": [{
                            "text": "Month of " + moment().format('MMMM'),
                            "bold": true
                        }],
              "dataProvider":data,
              "valueAxes":[{
        ...
        daily_gross_sales: function(company_id){
            var self = this;
            var today = moment().format('YYYY-MM-DD');
            var last_today_day = moment().subtract(7, "days").format('YYYY-MM-DD');
            var weekDayNameToday =  moment(today).format('dddd');
            var weekDayNameLastToday =  moment(last_today_day).format('dddd');
            ...
            rpc.query({
                ...
                var hrs_pay_data = []
                var today_day = moment().format('dddd, MMMM Do YYYY');
                var last_day = moment().subtract(7, "days").format('dddd, MMMM Do YYYY');
        ...
        weekly_gross_salse : function(company_id){
            var self = this;
            var current_week_start_date = moment().startOf('week').format('YYYY-MM-DD');
            var current_week_end_date = moment().endOf('week').format('YYYY-MM-DD');
            ...
            rpc.query({
                ...
                if(res && res['weekly_compare_sales'].length > 0){
                    var this_week = [moment(current_week_start_date).format('MMM.DD') + ' - ' + moment(current_week_end_date).format('MMM.DD')]
                    var las_week = [moment(last_week_start_date).format('MMM.DD') + ' - ' + moment(last_week_end_date).format('MMM.DD')]
        ...
        onchange_month_option : function(co){
            var self = this;
            var selectemonth = self.$el.find(".month-option option:selected").val();
            const firstDayOfMonth = moment('2018-'+selectemonth+"'", 'YYYY-MM-DD');
        ...
        employee_work_hour : function(company_id){
            var self = this;
            setTimeout(function(){
            var first = moment().startOf('week').format('YYYY-MM-DD');
            var last = moment().endOf('week').format('YYYY-MM-DD');
            rpc.query({
                model: 'pos.order',
        ...
        customer_avg_spent_per_visit : function (company_id){
            ...
            var end_date = moment().subtract(30, 'days').format('YYYY-MM-DD');;
            rpc.query({
                model: 'pos.order',
        ...
        header_data : function(company_id){
            ...
            rpc.query({
                ...
                if(res){
                    ...
                    self.$el.find('.active-session').empty()
                    self.$el.find('.active-session').append(res['session'])
                    self.$el.find('.total-orders').empty()
                    self.$el.find('.total-orders').append(res['order'])
                    self.$el.find('.total-sales').empty()
                    self.$el.find('.total-sales').append(res['total_sale'])
                    self.$el.find('.product-sold').empty()
                    self.$el.find('.product-sold').append(res['product_sold'])
                    self.$el.find('.today-total-sales').empty()
                    self.$el.find('.today-total-sales').append(res['today_sales'])
                    self.$el.find('.today-total-orders').empty()
                    self.$el.find('.today-total-orders').append(res['today_order'])
                    self.$el.find('.today-product-sold').empty()
                    self.$el.find('.today-product-sold').append(res['today_product'])
                }
            });
        },
